...
class ProductView(View):
    def get(self, request):
        ...
        gents = Product.objects.filter(category='G')
        mobiles = Product.objects.filter(category='M')
        laptops = Product.objects.filter(category='L')
        footwares = Product.objects.filter(category='F')
        bags = Product.objects.filter(category='B')
        return render(request, 'shop/home.html', {'ladies': ladies, 'gents': gents,
                                                  'mobiles': mobiles, 'laptops': laptops, 'footwares': footwares, 'bags': bags})


class productDetailView(View):
    def get(self, request, pk):
        product = Product.objects.get(pk=pk)
        advanced_item = False  # Already in cart section then logic is applied
        if request.user.is_authenticated:
            advanced_item = Cart.objects.filter(
                Q(product=product.id) & Q(user=request.user)).exists()
        return render(request, 'shop/productdetail.html', {'product': product, 'advanced_item': advanced_item})
...
def show_cart(request):
    if request.user.is_authenticated:
        ...
        ) if p.user == user]  # list comprehension
        if cart_product:
...
def orders(request):
    ...
    return render(request, 'shop/orders.html', {'order_placed': order_placed})
...
def payment_done(request):
    user = request.user
    custid = request.GET.get('custid')  # id from template
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            reg = Customer(user=usr, name=name, locality=locality,
                           city=city, zipcode=zipcode, district=district)
            reg.save()
